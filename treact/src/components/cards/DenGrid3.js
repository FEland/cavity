...
const SearchContainer = styled.div`
  ...
  .search-input {
    ${tw`w-3/4 py-3 pl-4 pr-2 text-gray-700 bg-transparent border-none focus:outline-none`}
    border-right: 2px solid #ccc; /* Add a border to separate the two inputs */
  }
...
// escape key registerd for when card popup open or  suggestion popup open
  ...
  useEffect(() => {
    // Your code here will run only once when the component mounts
    fetchUserLocation();
...
<SearchContainer>
      <input
          type="text"
          placeholder={`Search by Dentist, Speciality, Procedure...`}
          value={search}
          onChange={(e) => resetSearch(e.target.value)}
          onMouseLeave={() => setHoveredIndex(null)}
